    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const likesResponse = await receiveExerciseList(
                    "/filter",
                    options
                );
            ...
            try {
                ...
                if (like && favorite) {
                    if (favID.length > 0 && likeID.length > 0) {
                        ...
                        const idComunes = likeID.filter((like) =>
                            favID.includes(like)
                        );
                        console.log("idcomunes", idComunes);

                        const exerciseFilterList = exercisesResponse.filter(
                            (exercise) => idComunes.includes(exercise.id)
                        );
                    ...
                    const exerciseFilterList = exercisesResponse.filter(
                        (exercise) => likeID.includes(exercise.id)
                    );

                    setExercises(exerciseFilterList);
                } else if (favorite) {
                    const exerciseFilterList = exercisesResponse.filter(
                        (exercise) => favID.includes(exercise.id)
                    );
                    setExercises(exerciseFilterList);
                } else {
                    setExercises(exercisesResponse);

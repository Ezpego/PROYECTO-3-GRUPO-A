...
const ForgottenPassword = () => {
    ...
    try {
        ...
        const response = await fetchData(`${urlRaiz}/users/forgottenPassword`, options);
...
// !y ese no trae ninguna propiedad .status, .statusText.
    ...
    <form>
        <label>
        Por favor, ingresa tu dirección de correo electrónico para recuperar tu contraseña:
        <input
            type="email"
            value={email}
        ...
        <button onClick={handleForgotPassword} className='Enviar'>
            {isPending ? <><Spinner /> Enviando...</> : 'Enviar Solicitud'}
        </button>
    ...
    <section>
        ...
        <Link to="/users/register" className='link'>Crear nueva cuenta</Link>
    </section>
    <section>
        <button onClick={handleReturnButton} className='Volver'>Volver al inicio de sesión</button>
    </section>
    </>
    );
};
